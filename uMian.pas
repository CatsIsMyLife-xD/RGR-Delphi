...
uses
	...
	FireDAC.Comp.DataSet, FMX.Memo.Types, FireDAC.Phys.SQLiteWrapper.Stat;
...
{$IF DEFINED(MSWINDOWS)}
	...
	fdcBase.Params.Values['Database'] := IncludeTrailingPathDelimiter
		(ExtractFilePath(ParamStr(0))) + 'tasks.sdb';
{$ELSE}
	fdcBase.Params.Values['Database'] := TPath.GetDocumentsPath + PathDelim +
		'tasks.sdb';
{$ENDIF}
end;
...
begin
	...
	begin
		AddListBoxItem(fdtTask.FieldByName('Date').AsString,
			fdtTask.FieldByName('Time').AsString,
			fdtTask.FieldByName('Length').AsFloat);
		fdtTask.Next;
	end;
end;
...
begin
	...
	fdQuery.Open;
	laDateTimeValue.Text := fdQuery.FieldByName('Date').AsString + ' ' +
		fdQuery.FieldByName('Time').AsString;
	laLenghtValue.Text := 'Количество дней ' + fdQuery.FieldByName('Length')
		.AsString ;
...
begin
	...
	// new
	try
		fdQuery.ExecSQL('Insert into Task (ID, Length, Date, Time, Water, Comment) '
			+ 'values (:ID, :L, :D, :T, :W, :C)',
			[lbTask.Count + 1, sbLengthNew.Value, DateToStr(deDateNew.Date),
			TimeToStr(teTimeNew.Time), swWaterNew.IsChecked.ToInteger,
			meCommentNew.Text]);
	except
		on e: Exception do
			ShowMessage(e.Message);
	...
	AddListBoxItem(DateToStr(deDateNew.Date), TimeToStr(teTimeNew.Time),
		sbLengthNew.Value);
